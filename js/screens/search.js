...
function entryHit(e, q) {
  ...
  for (const a of e.aliases || []) if (re.test(a)) return { label: '별명', ...at(a) };
  for (const f of e.fields) {
    if (re.test(f.value)) return { label: f.label || '설정', ...at(f.value) };
    for (const c of f.changes || []) {
      if (re.test(c.value)) return { label: `${f.label || '설정'} (${db.chapters.get(c.chapterId)?.title || '?'}부터)`, ...at(c.value) };
    }
    if (re.test(f.label)) return { label: '항목', ...at(`${f.label}: ${f.value}`) };
  }
  if (re.test(e.note)) return { label: '메모', ...at(e.note) };
  return undefined;
}
...
export function searchScreen({ wid }) {
  ...
  const input = h('input', {
    ...
    placeholder: '원고와 설정에서 찾기', enterkeyhint: 'search', 'aria-label': '검색어',
  });
  ...
  function draw() {
    ...
    if (!q) {
      results.replaceChildren(h('p', { class: 'muted search-hint' }, '원고의 문장과 설정 자료(캐릭터·장소 등)를 한 번에 찾아요.'));
      return;
    }
    const re = findRe(q);
    ...
    const msSec = groups.length && h('section', { class: 'entry-sec' },
      h('h3', null, '원고', h('span', { class: 'count' }, `${num(total)}곳`)),
      groups.map(({ c, hits }) => h('div', { class: 'result-group' },
        h('button', { class: 'result-head', onclick: () => open(c, hits[0]) },
          h('span', null, c.title), h('span', { class: 'muted' }, `${hits.length}곳`)),
        ...
        hits.length > PER_CHAPTER
          ? h('button', { class: 'result more', onclick: () => open(c, hits[PER_CHAPTER]) }, `이 화에서 ${hits.length - PER_CHAPTER}곳 더 보기`)
          : null)));
    ...
    const loreSec = found.length && h('section', { class: 'entry-sec' },
      h('h3', null, '설정', h('span', { class: 'count' }, found.length)),
    ...
    results.replaceChildren(...(msSec || loreSec
      ...
      : [h('p', { class: 'muted search-hint' }, `‘${q}’${josa(q, '은', '는')} 원고에도 설정에도 없어요.`)]));
  }
