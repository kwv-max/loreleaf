// 오늘 쓴 글자 수. 화마다 '오늘 처음 손댈 때의 글자 수'를 기준으로 늘어난 만큼만 센다 (줄었으면 0).
...
function load() {
  ...
  return s && s.date === dayKey() ? s : { date: dayKey(), base: {}, len: {}, work: {} };
}
...
export function noteOpen(ch) {
  const s = load();
  if (!(ch.id in s.base)) { const n = lengthOf(ch); s.base[ch.id] = n; s.len[ch.id] = n; s.work[ch.id] = ch.workId; store(s); }
}
